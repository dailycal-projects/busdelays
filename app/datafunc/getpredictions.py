...
routes = ['6', '7', '12', '18', '36', '51B', '52', '65', '67', '79', '88', '800', '851', 'F']
prefix = "~/actransit/predictions/"
    ...
    for pred in tree:
        ...
        for direction in pred:
            if direction.tag == "message":
                continue
            filename = prefix + stopId + "_" + r + "_" + direction.attrib['title'] + ".csv"
            output = ""
            try:
                f = open(filename, 'r')
                output += f.read()
...
def main():
    start = time.time()
    print(os.getcwd())
    with open(prefix+"localstops.csv", 'r') as csvfile:
